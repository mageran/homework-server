...
const chemicalElement = symbol => {
    const element = PeriodicTable.elements.filter(element => element.symbol === symbol)[0];
    if (!element) {
        throw `no element with symbol "${symbol}" found in periodic table`;
    }
    return element;
}
...
const chemicalElementsAlphabetically = PeriodicTable.elements.sort((elem1, elem2) => elem1.symbol < elem2.symbol ? -1 : elem1.symbol > elem2.symbol ? 1 : 0);